...
export const extractBucketParams = (event: S3Event): S3Params[] =>
...
export const getTurns = (json) => {
  ...
  if (json.results.channel_labels) {
    channelZero = json.results.channel_labels.channels[0].items.map((item) => ({
      ...item,
      channel: 0,
    }));
    channelOne = json.results.channel_labels.channels[1].items.map((item) => ({
      ...item,
      channel: 1,
    }));
  } else {
    ...
    channelZero = segments
      .filter((segment) => segment.speaker_label === "spk_0")
      .map((item) => ({ ...item, speaker_label: 0 }));
    channelOne = segments
      .filter((segment) => segment.speaker_label === "spk_1")
      .map((item) => ({ ...item, speaker_label: 1 }));
  }
  const concatted = channelZero.concat(channelOne);
  let combined = concatted.map((word, i) => {
    if (!word.start_time)
      return { ...word, start_time: concatted[i - 1].start_time };
    else return word;
  });
  ...
  const turns = labeledByTurn.reduce(
    (turns, word) => ({
      ...turns,
  ...
  try {
    ...
    const tableUpdates = await documentClient
      .update({
        ...
        UpdateExpression:
          "set #status = :st, #fullTranscriptURI = :ftu, #transcriptURI = :tu",

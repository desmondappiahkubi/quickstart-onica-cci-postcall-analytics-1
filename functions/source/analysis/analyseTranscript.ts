...
const extractBucketParams = (event: S3Event): S3Params[] =>
...
const getDominantScore = (turn) => {
  ...
  if (
    turn.SentimentScore.Negative > NEGATIVE_SENTIMENT_THRESHOLD &&
    turn.SentimentScore.Positive > POSITIVE_SENTIMENT_THRESHOLD
  ) {
    if (turn.SentimentScore.Negative > turn.SentimentScore.Positive) {
      dominantSentiment = {
        Sentiment: "NEGATIVE",
        SentimentScore: turn.SentimentScore.Negative,
      };
    } else {
      dominantSentiment = {
        Sentiment: "POSITIVE",
        SentimentScore: turn.SentimentScore.Positive,
      };
    }
...
const getSpeaker = (turn) =>
  turn.hasOwnProperty("channel") ? turn.channel : turn.speaker_label;

const getSplit = (json, agentChannel) => {
  const turns = json.turns;
  const customerChannel = agentChannel === 0 ? 1 : 0;
  const agent = turns
    .filter((turn) => getSpeaker(turn) === agentChannel)
    .map((turn) => turn.text)
    .join("|");
  const customer = turns
    .filter((turn) => getSpeaker(turn) === customerChannel)
    .map((turn) => turn.text)
    .join("|");
...
export const handler = async (event: S3Event, context: Context) => {
  ...
  const resolutionEndpointArn = process.env.CALL_RESOLUTION_ENDPOINT_ARN.replace(
    "region",
    region
  ).replace("account-id", accountId);
  const motivationEndpointArn = process.env.CALL_MOTIVATION_ENDPOINT_ARN.replace(
    "region",
    region
  ).replace("account-id", accountId);
  try {
    ...
    const customerMatches = (customerTranscript || "")
      .split("|")
      .filter((turn) => turn.match(threatRegex));
    ...
    const customerParams = {
      TextList: [].concat(
        ...chunkDocuments(customerTranscript.replace("|", ""))
      ),
      LanguageCode: "en",
    };

    const callResolutionEndpointResult = await comprehend
      .describeEndpoint({
        EndpointArn: resolutionEndpointArn,
      })
      .promise();
    const calMotivationEndpointResult = await comprehend
      .describeEndpoint({
        EndpointArn: motivationEndpointArn,
      })
      .promise();

    const callResolutionEnpointProps =
      callResolutionEndpointResult.EndpointProperties;
    const callMotivationEndpointProps =
      calMotivationEndpointResult.EndpointProperties;
    ...
    if (callResolutionEnpointProps.Status === "IN_SERVICE") {
      ...
      const crStatus = await comprehend
        .classifyDocument({
          EndpointArn: resolutionEndpointArn,
    ...
    if (callMotivationEndpointProps.Status === "IN_SERVICE") {
      ...
      const cmStatus = await comprehend
        .classifyDocument({
          EndpointArn: motivationEndpointArn,
    ...
    while (agentTranscriptChunks.length !== 0) {
      ...
      if (textList.length !== 0) {
        agentSentimentBatchResponse.push(
          await comprehend
    ...
    while (customerTranscriptChunks.length !== 0) {
      ...
      if (textList.length !== 0) {
        customerSentimentBatchResponse.push(
          await comprehend
            .batchDetectSentiment({
              TextList: textList,
              LanguageCode: "en",
    ...
    const customerSentiment = getDominantSentiments(
      customerSentimentBatchResponse
    );

    turns.turns
      .filter(
        (t) =>
          t.channel == customerChannel || t.speaker_label == customerChannel
      )
      .map((t, i) => (t.sentiment = customerSentiment[i]));
    turns.turns
      .filter(
        (t) => t.channel == agentChannel || t.speaker_label == agentChannel
      )
      .map((t, i) => (t.sentiment = agentSentiment[i]));

    console.log(`Searching for ${decodeURIComponent(objectPath)}`);
    const existingFiles = await documentClient
      .query({
    ...
    const analysisResponse = template
      .replace("<AssetId>", JSON.stringify(assetId))
      .replace("<Turns>", JSON.stringify(turns.turns))
      .replace(
        "<CustomerEntities>",
        JSON.stringify(
          customerEntities.ResultList.filter(
      ...
      .replace(
        "<CustomerKeyPhrases>",
        JSON.stringify(
          customerKeyPhrases.ResultList.filter(
            (item) => item.KeyPhrases.length != 0
          ),
          null
        )
      )
      .replace(
        "<AgentEntities>",
        JSON.stringify(
          agentEntities.ResultList.filter((item) => item.Entities.length != 0),
          null
        )
      )
      .replace(
        "<AgentKeyPhrases>",
        JSON.stringify(
          agentKeyPhrases.ResultList.filter(
            (item) => item.KeyPhrases.length != 0
          ),
          null
        )
      ...
      .replace("<META>", JSON.stringify(existingFiles.Items[0].meta));
    ...
    const tableUpdates = await documentClient
      .update({
        TableName: process.env.TABLE_NAME,
        Key: {
